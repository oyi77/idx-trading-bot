...
class BotHandlers:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif cmd.intent == Intent.SIGNAL_SWING:
            await self.signal_swing(update, context)
        elif cmd.intent == Intent.SIGNAL_SCALP:
            await self.signal_scalp(update, context)
        elif cmd.intent == Intent.BANDARMOLOGY:
            await self.bandarmology(update, context)
        elif cmd.intent == Intent.EVENT:
            # Re-inject event text as args
            event_text = cmd.params.get("text", "")
            if event_text:
                context.args = event_text.split()
            else:
                context.args = []
            await self.event(update, context)
        elif cmd.intent == Intent.REPORT:
            await self.report(update, context)
        elif cmd.intent == Intent.JEJAK:
            await self.jejak(update, context)
        elif cmd.intent == Intent.LEADERBOARD:
            await self.leaderboard(update, context)
        elif cmd.intent == Intent.POINTS:
            await self.points(update, context)
        elif cmd.intent == Intent.PORTFOLIO:
            await self.portfolio(update, context)
        elif cmd.intent == Intent.PREMARKET:
            await self.premarket(update, context)
        else:
            await update.message.reply_text(
