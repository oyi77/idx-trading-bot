  ...
  <div class="container">
    ...
    <div class="pricing-grid">
      ...
      <div class="glass price-card featured reveal" style="transition-delay:.15s">
        ...
        <button class="mag-btn mag-btn-primary price-btn" onclick="buyPlan('pro')">Upgrade Pro →</button>
      </div>
      ...
      <div class="glass price-card reveal" style="transition-delay:.3s">
        ...
        <button class="mag-btn mag-btn-outline price-btn" onclick="buyPlan('premium')">Upgrade Premium →</button>
      </div>
      ...
      <div class="glass price-card reveal" style="transition-delay:.45s">
        ...
        <button class="mag-btn mag-btn-outline price-btn" onclick="buyPlan('lifetime')">Ambil Lifetime →</button>
      </div>
    </div>
  </div>
</section>
...
function buyPlan(t){
  const val={pro:79900,premium:149900,lifetime:1999900}[t]||79900;
  if(typeof fbq==='function'){fbq('track','Lead',{content_name:t,value:val,currency:'IDR'});fbq('track','InitiateCheckout',{content_name:t,value:val,currency:'IDR',num_items:1})}
  document.getElementById('checkoutTierName').textContent=t.charAt(0).toUpperCase()+t.slice(1)+' — Rp '+val.toLocaleString('id');
...
async function submitCheckout(e){
  ...
  try{
    const res=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,phone:phone,tier:selectedTier})});
    const order=await res.json();
    if(order.payment_url||order.public_order_url){
      if(typeof fbq==='function')fbq('track','Purchase',{value:{pro:79900,premium:149900,lifetime:1999900}[selectedTier],currency:'IDR',content_name:selectedTier});
      window.location.href=order.payment_url||order.public_order_url;
    }else{alert('Gagal: '+(order.message||JSON.stringify(order)));btn.textContent='Bayar Sekarang';btn.disabled=false}
  }catch(err){alert('Error: '+err.message);btn.textContent='Bayar Sekarang';btn.disabled=false}
}
document.getElementById('checkoutModal').addEventListener('click',function(e){if(e.target===this)closeCheckout()});
</script>
<!-- CHECKOUT MODAL -->
<div id="checkoutModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;justify-content:center;align-items:center;padding:20px">
  <div style="position:relative;width:100%;max-width:440px;background:linear-gradient(135deg,#0a0a14,#1a1f3a);border-radius:16px;border:1px solid rgba(255,255,255,.06);padding:32px">
    <button onclick="closeCheckout()" style="position:absolute;top:16px;right:16px;background:none;border:none;color:#94a3b8;font-size:24px;cursor:pointer">&times;</button>
    <h3 style="color:#f1f5f9;font-size:1.3rem;margin-bottom:4px">💳 Checkout — <span id="checkoutTierName">Pro</span></h3>
    <p style="color:#94a3b8;font-size:.85rem;margin-bottom:20px">Isi data untuk melanjutkan pembayaran</p>
    <form onsubmit="submitCheckout(event)">
      <label style="display:block;color:#cbd5e1;font-size:.82rem;margin-bottom:4px">Username Telegram</label>
      <input id="checkoutName" type="text" placeholder="@username_kamu" required style="width:100%;padding:12px 16px;background:rgba(10,10,20,.6);border:1px solid rgba(255,255,255,.06);border-radius:10px;color:#f1f5f9;font-size:.95rem;margin-bottom:14px;outline:none">
      <label style="display:block;color:#cbd5e1;font-size:.82rem;margin-bottom:4px">No. WhatsApp</label>
      <input id="checkoutPhone" type="tel" placeholder="08xxxxxxxxxx" required style="width:100%;padding:12px 16px;background:rgba(10,10,20,.6);border:1px solid rgba(255,255,255,.06);border-radius:10px;color:#f1f5f9;font-size:.95rem;margin-bottom:20px;outline:none">
      <button id="checkoutBtn" type="submit" style="width:100%;padding:14px;background:linear-gradient(135deg,#00e5ff,#a855f7);color:#000;font-weight:700;font-size:1rem;border:none;border-radius:10px;cursor:pointer">Bayar Sekarang</button>
      <p style="color:#64748b;font-size:.75rem;text-align:center;margin-top:12px">🛡️ Pembayaran aman via ScaleV · 7 hari uang kembali</p>
    </form>
  </div>
</div>
</body>
